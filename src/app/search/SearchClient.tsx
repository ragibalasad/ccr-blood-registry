...
export default function SearchClient({
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 items-start w-full min-w-0">
      ...
      {/* ── Mobile Filter Bar ── */}
      <div className="md:hidden w-screen -mx-4 px-4 sticky top-16 z-30 bg-slate-50 pt-1 pb-3 border-b border-slate-200 space-y-2">
      ...
      <div className="hidden md:flex w-56 shrink-0 flex-col gap-2 sticky top-24 bg-slate-50">
        ...
        <button
          onClick={() => handleSearch("")}
          className={`w-full px-3 py-2 text-sm text-left rounded-md transition-colors font-medium border ${query === ""
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          {bloodGroups.map(bg => (
            <button
              key={bg}
              onClick={() => handleSearch(bg)}
              className={`py-2 px-2 text-sm text-center rounded-md font-bold transition-all border ${query === bg
